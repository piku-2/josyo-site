...
- **公開 URL**: https://jyousho-official.github.io/jyousho-official-site/
...
GitHub Actions が自動でビルド & 公開   ← ここは全自動。何もしなくてよい
        │
        ▼
https://jyousho-official.github.io/jyousho-official-site/ が更新される（数分後）
```
...
`public/logo.png` を置いたら、`index.astro` の中で `<img src="/jyousho-official-site/logo.png" />` のように呼び出せます。

> ⚠️ パスの先頭に `/jyousho-official-site/` が必要です（理由は「8. よくある質問」参照）。
...
| ファイル | できる URL |
| :-- | :-- |
| `src/pages/index.astro` | `/jyousho-official-site/`（トップ） |
| `src/pages/about.astro` | `/jyousho-official-site/about/` |
...
実行すると `http://localhost:4321/jyousho-official-site/` が表示されます。
...
**数分待つと** https://jyousho-official.github.io/jyousho-official-site/ に反映されます。
...
**Q. なぜ画像やリンクのパスに `/jyousho-official-site/` が付くの？**
...
- **管理者の代替わり**は、組織の `People` タブで次期管理者を **Owner** ロールにするだけで完結します。
  一般部員は **Member** のまま追加すればOKです（権限の渡し方は §6 と同じ考え方）。
...
- **リポジトリ名を変えると公開URLも変わります**。その場合は `astro.config.mjs` の `base` を
  `/新リポジトリ名/` に直してください（組織名を変えたら `site` も直す）。
